...
OUTPUT_FILE = OUTPUT_DIR / "dgl_subgraphx_shared_graphsage_results.csv"
PARTIAL_FILE = OUTPUT_DIR / "dgl_subgraphx_shared_graphsage_partial.csv"
...
class EgoGraphClassifier(nn.Module):
    ...
    The AML model is node classification:
        model(x, edge_index) -> node-level logits

    This wrapper returns the target node's logits as graph-level logits.
    """
...
def run_subgraphx_for_node(
    ...
    print("\n" + "-" * 80)
    print(f"DGL SubgraphX shared node_id={node_id}")
    print("-" * 80)
    ...
    if wrapped_pred != pred_label:
        print(
            ...
            "This row will still be saved, but mark wrapped_matches_full_prediction=False."
        )
...
def main():
    ...
    print("Running shared-node official DGL SubgraphX on fixed GraphSAGE checkpoint.")
    ...
    for i, node_id in enumerate(node_ids, start=1):
        print("\n" + "=" * 80)
        print(f"SubgraphX node {i}/{len(node_ids)}")
        print("=" * 80)
