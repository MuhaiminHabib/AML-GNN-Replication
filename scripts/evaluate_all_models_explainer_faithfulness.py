...
EXPLAINER_INPUTS = [
    ("GNNExplainer", "gnnexplainer_shared_{model}_results.csv"),
    ("PGExplainer", "pgexplainer_shared_{model}_results.csv"),
    ("DGL_SubgraphX", "dgl_subgraphx_shared_{model}_results.csv"),
]

DETAIL_OUTPUT = OUTPUT_DIR / "all_models_explainer_faithfulness.csv"
SUMMARY_OUTPUT = OUTPUT_DIR / "all_models_explainer_faithfulness_summary.csv"
...
def load_model_explainer_outputs(model_name: str):
    frames = []

    for expected_explainer, file_pattern in EXPLAINER_INPUTS:
        path = OUTPUT_DIR / file_pattern.format(model=model_name)
        ...
        if "explainer" not in df.columns:
            df["explainer"] = expected_explainer
...
def summarise_results(detail_df):
    ...
    summary["explainer_order"] = summary["explainer"].map(explainer_order)

    summary = (
        summary
        .sort_values(["model_order", "explainer_order"])
        .drop(columns=["model_order", "explainer_order"])
    )
...
def main():
    ...
    print("Evaluating faithfulness for all models and all explainers.")
    ...
    for model_name in MODELS:
        ...
        model_outputs = load_model_explainer_outputs(model_name)
        ...
        for _, row in model_outputs.iterrows():
            ...
            all_detail_rows.append(result)

            print(
                f"{result['model']} | {result['explainer']} | "
