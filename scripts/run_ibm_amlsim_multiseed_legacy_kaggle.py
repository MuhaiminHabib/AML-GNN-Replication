...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "ibm_amlsim_legacy_kaggle"
...
# This matches your previous table:
# 10,000 nodes -> 6,500 train, 1,500 validation, 2,000 test
TRAIN_RATIO = 0.65
VAL_RATIO = 0.15
TEST_RATIO = 0.20
...
# IMPORTANT:
# This legacy setting includes fraud-derived transaction-count features.
# It is useful for reproducing your previous Kaggle-style results, but it should
# be clearly separated from the leakage-free AMLSim experiment.
INCLUDE_FRAUD_TX_FEATURES = True
...
def train_one_model(seed: int, model_name: str, device: torch.device):
    print("\n" + "=" * 100)
    print(f"Dataset: IBM AMLSim Legacy Kaggle | Model: {model_name} | Seed: {seed}")
    print("=" * 100)
    ...
    result = {
        ...
        "num_node_features": split_summary["num_node_features"],
        "num_edge_features": split_summary["num_edge_features"],
        "num_transactions": split_summary["num_transactions"],
        "num_fraud_transactions": split_summary["num_fraud_transactions"],
        "num_fraud_accounts": split_summary["num_fraud_accounts"],
        "train_nodes": split_summary["train_nodes"],
        "val_nodes": split_summary["val_nodes"],
        "test_nodes": split_summary["test_nodes"],
        "train_fraud_accounts": split_summary["train_fraud_accounts"],
        "val_fraud_accounts": split_summary["val_fraud_accounts"],
        "test_fraud_accounts": split_summary["test_fraud_accounts"],
    ...
    history_path = (
        OUTPUT_DIR
        / f"ibm_amlsim_legacy_{model_name_safe}_seed{seed}_history.csv"
    )

    checkpoint_path = (
        OUTPUT_DIR
        / f"ibm_amlsim_legacy_{model_name_safe}_seed{seed}_model.pt"
    )
...
def main():
    ...
    print(f"Include fraud transaction features: {INCLUDE_FRAUD_TX_FEATURES}")
    print(f"Split: train={TRAIN_RATIO}, val={VAL_RATIO}, test={TEST_RATIO}")

    all_results = []

    partial_path = OUTPUT_DIR / "ibm_amlsim_legacy_multiseed_partial_results.csv"
    ...
    summary_df = (
        ...
        .agg(
            runs=("seed", "nunique"),
            train_nodes=("train_nodes", "first"),
            val_nodes=("val_nodes", "first"),
            test_nodes=("test_nodes", "first"),
            train_fraud_accounts=("train_fraud_accounts", "first"),
            val_fraud_accounts=("val_fraud_accounts", "first"),
            test_fraud_accounts=("test_fraud_accounts", "first"),
    ...
    results_path = OUTPUT_DIR / "ibm_amlsim_legacy_multiseed_results.csv"
    summary_path = OUTPUT_DIR / "ibm_amlsim_legacy_multiseed_summary.csv"

    results_df.to_csv(results_path, index=False)
    summary_df.to_csv(summary_path, index=False)

    print("\n" + "=" * 100)
    print("IBM AMLSim legacy Kaggle multi-seed summary")
    print("=" * 100)
