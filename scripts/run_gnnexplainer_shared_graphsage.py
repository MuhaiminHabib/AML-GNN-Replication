...
OUTPUT_FILE = OUTPUT_DIR / "gnnexplainer_shared_graphsage_results.csv"
...
def summarise_explanation(explanation, data, node_id, probs_cpu, pred_cpu, checkpoint):
...
def main():
    ...
    print("Running shared-node GNNExplainer on fixed GraphSAGE checkpoint.")
    ...
    for i, node_id in enumerate(node_ids, start=1):
        print("\n" + "-" * 80)
        print(f"GNNExplainer node {i}/{len(node_ids)} | node_id={node_id}")
        print("-" * 80)
        ...
        pd.DataFrame(rows).to_csv(
            OUTPUT_DIR / "gnnexplainer_shared_graphsage_partial.csv",
            index=False,
        )
