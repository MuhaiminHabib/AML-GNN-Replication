...
class GATv2(nn.Module):
    def __init__(self, in_channels, hidden_channels, out_channels=2, dropout=0.5):
        super().__init__()
        # heads=1 to keep memory manageable on the 9.5M-edge graph
        self.conv1 = GATv2Conv(in_channels, hidden_channels, heads=1, dropout=dropout)
        self.conv2 = GATv2Conv(hidden_channels, out_channels, heads=1, concat=False, dropout=dropout)
        self.dropout = dropout
...
def main():
    ...
    # Start with GCN and GraphSAGE. GATv2 may be slower/heavier on 9.5M edges.
    ...
    for model_name in models:
        for seed in SEEDS:
            ...
            partial_df.to_csv(
                OUTPUT_DIR / "saml_d_multiseed_partial_results.csv",
                index=False,
            )
    ...
    results_path = OUTPUT_DIR / "saml_d_multiseed_results.csv"
    ...
    summary_path = OUTPUT_DIR / "saml_d_multiseed_summary.csv"
    summary_df.to_csv(summary_path, index=False)

    print("\nSAML-D multi-seed summary")
    print("=" * 80)
