...
OUTPUT_DIR = PROJECT_ROOT / "outputs" / "ibm_amlsim"
...
def train_one_model(seed: int, model_name: str, device: torch.device):
    ...
    history_path = (
        OUTPUT_DIR
        / f"ibm_amlsim_{model_name.lower()}_seed{seed}_history.csv"
    )
    ...
    checkpoint_path = (
        OUTPUT_DIR
        / f"ibm_amlsim_{model_name.lower()}_seed{seed}_model.pt"
    )
...
def main():
    ...
    summary_df = (
        results_df
        .groupby(["dataset", "task", "label_source", "temporal_split", "model"], as_index=False)
        .agg(
            runs=("seed", "nunique"),
