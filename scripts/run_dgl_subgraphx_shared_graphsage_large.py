...
OUTPUT_FILE = OUTPUT_DIR / "dgl_subgraphx_shared_graphsage_large_results.csv"
PARTIAL_FILE = OUTPUT_DIR / "dgl_subgraphx_shared_graphsage_large_partial.csv"
...
# Larger SubgraphX search budget for sensitivity check.
SUBGRAPHX_NUM_HOPS = 2
SUBGRAPHX_NUM_CHILD = 10
SUBGRAPHX_NUM_ROLLOUTS = 10
SUBGRAPHX_NODE_MIN = 3
SUBGRAPHX_SHAPLEY_STEPS = 10
...
class EgoGraphClassifier(nn.Module):
    ...
    This wrapper converts the node-classification model into a graph-level
    model by returning the target node's logits as the graph prediction.
    """
...
def extract_pyg_ego_to_dgl(data, node_id: int, num_hops: int):
    ...
    # DGL SubgraphX is more stable when self-loops are explicit.
...
def explanation_edges_from_nodes(dgl_graph, explanation_local_nodes):
    """
    SubgraphX returns explanatory nodes.
    For our shared faithfulness evaluator, we map those nodes back into
    induced edges inside the DGL ego graph.
    """
    explanation_set = set(
...
def run_subgraphx_for_node(
    ...
    print("\n" + "-" * 80)
    print(f"DGL SubgraphX large shared node_id={node_id}")
    print("-" * 80)
    ...
    if not wrapped_matches:
        print(
            ...
            "This row will still be saved with wrapped_matches_full_prediction=False."
        )
...
def main():
    ...
    # Use CPU because this runs inside the DGL environment.
    device = torch.device("cpu")

    print(f"Using device: {device}")
    print("Running shared-node official DGL SubgraphX large-budget sensitivity check.")
    ...
    for i, node_id in enumerate(node_ids, start=1):
        print("\n" + "=" * 80)
        print(f"SubgraphX large node {i}/{len(node_ids)}")
        print("=" * 80)
    ...
    print("\nDGL SubgraphX large shared summary")
    print("=" * 80)
    ...
    if len(final_df) > 0:
        ...
        print("No DGL SubgraphX large explanations were produced.")

    print(f"\nSaved shared DGL SubgraphX large results to: {OUTPUT_FILE}")
